...
dataframeRating = pd.read_json("ratings.json")

dataframeRating.fillna(0, inplace=True)
user_ratings = pd.merge(dataframeUser, dataframeRating, on="UserId")
user_ratings_data = pd.merge(user_ratings,dataframeBook, on="ISBN")

test = pd.DataFrame(dataframeRating.groupby('ISBN')['Rating'].mean());
test['rating_count'] = pd.DataFrame(dataframeRating.groupby('ISBN')['Rating'].count());
